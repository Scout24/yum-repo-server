...
## Getting started
* Dependencies: <code>sudo apt-get install python-pycurl python-yaml python-lxml python-django python-django-piston python-nose createrepo python-rpm python-daemon python-lockfile python-stdeb</code> and <code>pypi-install APScheduler ; pypi-install teamcity-messages</code> as these modules are not part of Ubuntu (see[http://pypi.python.org/pypi/stdeb] for more info).
